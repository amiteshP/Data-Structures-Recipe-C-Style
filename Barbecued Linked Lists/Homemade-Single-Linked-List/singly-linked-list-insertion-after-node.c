//The program discusses how to insert a new node after a specified node in the Linked-List
...
struct node *insert_after(struct node *start, int data, int item); //Function declaration so program knows that it'll be used in the code.
...
int main(){
	...
	int item;						//Variable we'll use to hold the value of node after which the new node is to be inserted.
	...
	scanf("%d", &item);
	start = insert_after(start, data, item);	//the function returns start, as if it's position is modified during the function
												//execution, we will know the new positiona s it's the only way to access the list.
												//The data to be inserted and the node-value after which to insert is passed as arguments.

	display(start);					//To display the newly modified list
	return 0;
}

struct node *insert_after(struct node *start, int data, int item){	//Actual function definition.
	
	struct node *new_node;						//Pointer to the new node we're adding.

	struct node *ptr;							//To access the pointer to the node after which we have to insert new node.
	...
	while(ptr!=NULL){						//Loop through till the end of list(node points to NULL)

		if(ptr->info == item){				//If the node after which new node is to be added is found.
	...
	printf("The item after which new node is to be entered is not present in the list.\n");
	
	return start;		//Return the start pointer even if the node after which new node is to be enetered is not found.
}
